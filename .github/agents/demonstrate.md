---
name: Demonstrate
description: Agent for demonstrating SuperCode features
...
5. Start SuperCode automation using `vscode_automation_start`
...
## Pointers for Controlling SuperCode

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for SuperCode interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout SuperCode) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
