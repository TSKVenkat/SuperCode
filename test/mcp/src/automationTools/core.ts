...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	tools.push(server.tool(
		...
		'Restart SuperCode with optional workspace or folder and extra command-line arguments',
		{
		...
		async ({ workspaceOrFolder, extraArgs }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: `SuperCode restarted successfully${workspaceText}${argsText}`
				}]
			};
		}
	));
	...
	tools.push(server.tool(
		...
		'Stop the SuperCode application',
		async () => {
			const app = await appService.getOrCreateApplication();
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'SuperCode stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of SuperCode (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 	'Send a keybinding to SuperCode (e.g., ctrl+shift+p, cmd+s)',
	// 	{
