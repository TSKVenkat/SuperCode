...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'SuperCode', separator: { label: ' - ' } }), 'SuperCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'SuperCode', separator: { label: ' - ' } }), 'Untitled-1 - SuperCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'SuperCode', separator: { label: ' - ' } }), 'monaco - SuperCode');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'SuperCode', separator: { label: ' - ' } }), 'somefile.txt - monaco - SuperCode');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'SuperCode', separator: { label: ' - ' } }), '* somefile.txt - monaco - SuperCode');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'SuperCode', separator: { label: ' - ' } }), 'monaco - SuperCode');
	});
